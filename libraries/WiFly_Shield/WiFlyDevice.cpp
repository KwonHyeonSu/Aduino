...
void WiFlyDevice::requireFlowControl() {
  ...
  sendCommand("set comm size 1460", false, "AOK");
...
boolean WiFlyDevice::join(const char *ssid, const char *passphrase, boolean isWPA) 
{
  ...
  boolean ret = sendCommand(charBuf2,false,"Associated!");
  return ret;
}
...
void WiFlyDevice::factoryReset()
{
    Serial.println("Wifi Factory Reset. wait.");
    
	enterCommandMode();
	uart->println("factory RESET");
